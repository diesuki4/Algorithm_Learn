...
string days_to_date(unsigned days);
string add_month(string date, unsigned month);
...
string days_to_date(unsigned days)
{
    unsigned day = days % 28;
    days /= 28;

    unsigned month = days % 12 + 1;
    unsigned year = days / 12;
...
string add_month(string date, unsigned month)
{
    return days_to_date(date_to_days(date) + month * 28);
}
...
vector<int> solution(string today, vector<string> terms, vector<string> privacies)
{
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        if (date_to_days(add_month(pvcy_date, umap[pvcy_type])) <= date_to_days(today))
